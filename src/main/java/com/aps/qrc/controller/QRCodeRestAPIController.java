...
/**
 * The controller will take the request and provide
 * response accordingly. It is just a door to the application. The user
 * can request using specific URL. In controller, the requests, types of
 * request and corresponding responses are specified. Here, we make two
 * requests and responses, the first one for generating QR Code and the
 * second one for reading QR Code.
 */
// RestController makes the application RestFul (get more information about RestFul web services)
@RestController
// the main mapping directory (root directory), other mapping requests will be appended to it
@RequestMapping("/qrcodeapi")
public class QRCodeRestAPIController {

    /*
     * [test] Checking the spring boot web service
     *
     * @RequestMapping("/check") public String check() { return "working..."; }
     */

    /*
     * log messages to determine the status of request and response to the API the
     * log will help us to track the application requests and responses Specially,
     * it is good for the developers, it can be also useful in case of security and
     * the application crashing in case of any problem. The log can be also stored
     * in a file or database, many configuration can be done on log messages.
     */
    public static final Logger logger = LoggerFactory.getLogger(QRCodeRestAPIController.class);

    /*
     * Using Autowired, we can can provide dependency injection, it means we can
     * inject a class without creating its instance, it helps us in loose coupling
     * and high cohesion.
     */
    ...
    /*
     * Request for reading QR Code image (reading its content, it can only read the
     * QR Code as per APS Payment QR Code standard.
     */
    // NOTE: we will write custom exception and error handling methods in next
    // version release (Handling invalid image, not image ,....)
